...
class AmbassadorAdminDashboard extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AmbassadorAdminDashboard> createState() => _AmbassadorAdminDashboardState();
}

class _AmbassadorAdminDashboardState extends ConsumerState<AmbassadorAdminDashboard>
  ...
  Widget _buildGlobalStatsCards(ThemeData theme) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
  ...
  Widget _buildUtilizationChart(ThemeData theme) {
    return Container(
  ...
  Widget _buildCountryDistribution(ThemeData theme) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        color: theme.colorScheme.surface,
  ...
  List<Widget> _buildCountryItems(ThemeData theme) {
    final countries = [
      {'code': 'US', 'name': 'United States', 'ambassadors': 287, 'quota': 345, 'growth': '+12'},
      {'code': 'DE', 'name': 'Germany', 'ambassadors': 98, 'quota': 133, 'growth': '+8'},
      {'code': 'FR', 'name': 'France', 'ambassadors': 89, 'quota': 142, 'growth': '+5'},
      {'code': 'IT', 'name': 'Italy', 'ambassadors': 76, 'quota': 144, 'growth': '+3'},
      {'code': 'ES', 'name': 'Spain', 'ambassadors': 134, 'quota': 220, 'growth': '+15'},
    ];

    return countries.map((country) {
      final utilization = (country['ambassadors'] as int) / (country['quota'] as int);
                      ...
                      Expanded(
                        child: LinearProgressIndicator(
                          ...
                          valueColor: AlwaysStoppedAnimation(
                            utilization > 0.8 ? Colors.green : 
                            utilization > 0.5 ? Colors.orange : Colors.red,
                          ),
                        ),
                      ),
  ...
  Widget _buildReferralTrends(ThemeData theme) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        color: theme.colorScheme.surface,
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: _buildTrendItem('Total Referrals', '2,847', '+12%', Colors.blue, theme),
              ),
              Expanded(
                child: _buildTrendItem('Avg per Ambassador', '8.3', '+5%', Colors.green, theme),
              ),
            ],
          ),
          ...
          Row(
            children: [
              Expanded(
                child: _buildTrendItem('Conversion Rate', '73%', '+2%', Colors.orange, theme),
              ),
              Expanded(
                child: _buildTrendItem('Active Rate', '87%', '-1%', Colors.red, theme),
              ),
            ],
          ),
        ],
  ...
  Widget _buildTrendItem(String label, String value, String change, Color color, ThemeData theme) {
        ...
        children: [
          ...
          ListView.separated(
            ...
            itemBuilder: (context, index) {
              return _buildPerformerItem(index + 1, theme);
            },
          ),
        ],
      ),
  ...
  Widget _buildPerformerItem(int rank, ThemeData theme) {
    ...
    final tier = rank <= 2 ? 'Lifetime' : rank <= 4 ? 'Premium' : 'Basic';
    final tierColor = rank <= 2 ? Colors.orange : rank <= 4 ? Colors.blue : Colors.green;
    ...
    return Row(
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            color: rank <= 3 ? 
              (rank == 1 ? Colors.amber : rank == 2 ? Colors.grey.shade400 : Colors.brown.shade400) :
  ...
  Widget _buildAutomationTab(ThemeData theme) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        children: [
  ...
  Widget _buildAutomationStatus(ThemeData theme) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        color: theme.colorScheme.surface,
  ...
  Widget _buildAutomationLogs(ThemeData theme) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        color: theme.colorScheme.surface,
  ...
  Widget _buildLogItem(int index, ThemeData theme) {
    final logs = [
      {'time': '14:30', 'action': 'Daily eligibility check completed', 'result': '12 promotions'},
      {'time': '12:45', 'action': 'Tier upgrade processed', 'result': 'User promoted to Premium'},
      {'time': '11:20', 'action': 'Monthly quota validation', 'result': 'All quotas within limits'},
      {'time': '09:15', 'action': 'Referral tracking updated', 'result': '234 new referrals processed'},
      {'time': '08:00', 'action': 'Performance metrics calculated', 'result': 'Global stats updated'},
      {'time': '06:30', 'action': 'Automation health check', 'result': 'All systems operational'},
      {'time': '04:15', 'action': 'Database cleanup completed', 'result': 'Inactive records archived'},
      {'time': '02:00', 'action': 'Scheduled eligibility check', 'result': '8 users promoted'},
    ];
