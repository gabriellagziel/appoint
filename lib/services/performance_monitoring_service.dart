...
class PerformanceMonitoringService {
  ...
  Future<void> startTrace(final String name,
      {final Map<String, dynamic>? attributes}) async {
  ...
  T measureExecutionSync<T>(final String name, final T Function() function,
      {final Map<String, dynamic>? attributes}) {
    final stopwatch = Stopwatch()..start();
  ...
  Future<T> measureExecutionAsync<T>(
      final String name, final Future<T> Function() function,
      {final Map<String, dynamic>? attributes}) async {
    final stopwatch = Stopwatch()..start();
  ...
  Map<String, dynamic> getMetrics() {
    ...
    metrics['operation_times'] = _operationTimes
        .map((final key, final value) => MapEntry(key, value.inMilliseconds));

    // Error counts
    metrics['error_counts'] = _errorCounts;

    // Trace history
    metrics['trace_history'] = _traceHistory.map((final key, final value) =>
        MapEntry(key, value.map((final d) => d.inMilliseconds).toList()));
...
final REDACTED_TOKEN =
    Provider<PerformanceMonitoringService>((final ref) {
