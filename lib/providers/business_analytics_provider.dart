...
final bookingsOverTimeProvider =
  ...
  return ref.read(REDACTED_TOKEN).fetchBookingsOverTime();
});

final serviceDistributionProvider =
    FutureProvider<List<ServiceDistribution>>((final ref) {
  return ref.read(REDACTED_TOKEN).fetchServiceDistribution();
});

final revenueByStaffProvider =
    FutureProvider<List<RevenueByStaff>>((final ref) {
  return ref.read(REDACTED_TOKEN).fetchRevenueByStaff();
});
