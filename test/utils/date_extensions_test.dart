...
void main() {
  group('DateExtensions', () {
    final tz = DateTime.utc; // easier in UTC
    ...
    group('daysDifference()', () {
      test('returns correct difference for same day', () {
        final d1 = tz(2025, 3, 9, 10, 0);
        final d2 = tz(2025, 3, 9, 20, 0);
    ...
    group('toRelativeTime()', () {
      test('returns "just now" for very recent times', () {
        ...
        if (timeDiff.inMinutes < 1) {
          ...
          expect(minutes == 1 ? '1 minute ago' : '$minutes minutes ago', '30 seconds ago');
        }
      });
      ...
      test('returns minutes ago for recent times', () {
        ...
        final timeDiff = recent.difference(now);
        final minutes = timeDiff.inMinutes.abs();
        expect(minutes == 1 ? '1 minute ago' : '$minutes minutes ago', '30 minutes ago');
      });
      ...
      test('returns singular minute correctly', () {
        ...
        final timeDiff = recent.difference(now);
        final minutes = timeDiff.inMinutes.abs();
        expect(minutes == 1 ? '1 minute ago' : '$minutes minutes ago', '1 minute ago');
      });

      test('returns hours ago for recent times', () {
        final now = tz(2025, 3, 9, 12, 0);
        final recent = tz(2025, 3, 9, 10, 0);
        final timeDiff = recent.difference(now);
