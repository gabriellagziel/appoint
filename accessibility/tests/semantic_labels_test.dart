...
class SemanticLabelsTest {
    ...
    for (int i = 0; i < tester.widgetList(iconButtons).length; i++) {
      ...
      if (!hasLabel) {
        issues.add(AccessibilityIssue(
          ...
          recommendation: 'Add semanticLabel property to describe the button action',
  ...
  static Future<AccessibilityTestResult> testFormFieldLabels(WidgetTester tester) async {
    final issues = <AccessibilityIssue>[];
    ...
    for (int i = 0; i < tester.widgetList(textFields).length; i++) {
      ...
      if (!hasLabel) {
        issues.add(AccessibilityIssue(
          ...
          recommendation: 'Add labelText or ensure field has descriptive placeholder',
          element: 'TextField[$i]',
        ));
      }
    }
    ...
    for (int i = 0; i < tester.widgetList(checkboxes).length; i++) {
      ...
      if (!hasLabel) {
        issues.add(AccessibilityIssue(
          ...
          recommendation: 'Wrap checkbox in CheckboxListTile or add semantic label',
          element: 'Checkbox[$i]',
        ));
      }
    }
  ...
  static Future<AccessibilityTestResult> testHeadingHierarchy(WidgetTester tester) async {
    final issues = <AccessibilityIssue>[];
    ...
    for (int i = 1; i < levels.length; i++) {
      if (levels[i] - levels[i - 1] > 1) {
        issues.add(AccessibilityIssue(
          ...
          description: 'Heading hierarchy has gap: ${levels[i - 1]} -> ${levels[i]}',
          recommendation: 'Ensure heading levels are sequential (h1, h2, h3, etc.)',
  ...
  static Future<AccessibilityTestResult> testImageAltText(WidgetTester tester) async {
    final issues = <AccessibilityIssue>[];
    ...
    for (int i = 0; i < tester.widgetList(images).length; i++) {
      ...
      if (!hasAltText) {
        issues.add(AccessibilityIssue(
          ...
          recommendation: 'Add semanticLabel or excludeSemantics for decorative images',
          element: 'Image[$i]',
        ));
      }
    }
  ...
  static Future<AccessibilityTestResult> testCustomWidgetLabels(WidgetTester tester) async {
    final issues = <AccessibilityIssue>[];
    ...
    for (int i = 0; i < tester.widgetList(customWidgets).length; i++) {
      ...
      if (!hasLabel) {
        issues.add(AccessibilityIssue(
          ...
          recommendation: 'Add semanticLabel if card contains important content',
          element: 'Card[$i]',
        ));
      }
    }
...
class AccessibilityTestResult {
  ...
  int get criticalIssues => issues.where((i) => i.severity == AccessibilityIssueSeverity.critical).length;
  int get highIssues => issues.where((i) => i.severity == AccessibilityIssueSeverity.high).length;
  int get mediumIssues => issues.where((i) => i.severity == AccessibilityIssueSeverity.medium).length;
  int get lowIssues => issues.where((i) => i.severity == AccessibilityIssueSeverity.low).length;
