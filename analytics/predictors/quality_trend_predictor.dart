...
class QualityTrendPredictor {
  ...
  static Future<QualityTrendPrediction> predictQualityTrend({
    ...
    // Generate forecast
    final forecast = _generateForecast(historicalMetrics, predictionWindow, trend);
  ...
  static TrendAnalysis _calculateTrend(List<QualityMetric> metrics) {
    ...
    final timestamps = metrics.map((m) => m.timestamp.millisecondsSinceEpoch).toList();
  ...
  static List<QualityMetric> _generateForecast(
    ...
    for (int day = 1; day <= daysToPredict; day++) {
      ...
      final predictedValue = trend.intercept + trend.slope * futureTimestamp.millisecondsSinceEpoch;
  ...
  static double _calculateConfidence(List<QualityMetric> metrics, TrendAnalysis trend) {
    final values = metrics.map((m) => m.value).toList();
    final variance = _calculateVariance(values);
    final trendStrength = (trend.slope * 1000).abs(); // Scale slope for confidence
