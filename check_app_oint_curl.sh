...
check_path() {
    ...
    # Get HTTP status and redirect info
    local response=$(curl -s -I -L -w "%{http_code}|%{redirect_url}|%{url_effective}" -o /dev/null "$url" 2>/dev/null)
    local status_code=$(echo "$response" | cut -d'|' -f1)
    local redirect_url=$(echo "$response" | cut -d'|' -f2)
    local final_url=$(echo "$response" | cut -d'|' -f3)
    
    # Get page content
    local content=$(curl -s -L "$url" 2>/dev/null)
    ...
    cat <<EOF
{
  ...
  "redirects_to": $(if [ "$redirect_url" != "" ]; then echo "\"$redirect_url\""; else echo "null"; fi),
  "title": "$title",
  "heading": "$heading",
  "ssl": $ssl_info
}
