...
exports.resetMonthlyQuotas = exports.businessApi = exports.registerBusiness = void 0;
// businessApi.ts
...
function generateApiKey() {
    ...
    return global.crypto?.randomUUID?.() || require('crypto').randomBytes(32).toString('hex');
}
/**
...
businessApiApp.use(ipWhitelist_1.ipWhitelistMiddleware);
businessApiApp.use(rateLimiter_1.rateLimitMiddleware);
businessApiApp.use(auditLogger_1.auditLogMiddleware);
// After body parser
...
async function logUsage({ businessId, endpoint, ip, success, latencyMs, }) {
    try {
        const logData = {
            businessId,
            endpoint,
...
businessApiApp.post('/appointments/create', async (req, res) => {
    ...
    try {
        ...
        await logUsage({ businessId, endpoint: 'appointments/create', ip: req.ip, success: true, latencyMs: Date.now() - req._startTime });
        res.status(201).json({
        ...
        await logUsage({ businessId, endpoint: 'appointments/create', ip: req.ip, success: false, latencyMs: Date.now() - req._startTime });
        res.status(500).json({ error: 'Internal Server Error' });
    }
});
...
businessApiApp.post('/appointments/cancel', async (req, res) => {
    ...
    try {
        ...
        await logUsage({ businessId, endpoint: 'appointments/cancel', ip: req.ip, success: true, latencyMs: Date.now() - req._startTime });
        res.status(200).json({
            cancelled: true,
            appointmentId,
            status: 'cancelled',
        ...
        await logUsage({ businessId, endpoint: 'appointments/cancel', ip: req.ip, success: false, latencyMs: Date.now() - req._startTime });
        res.status(500).json({ error: 'Internal Server Error' });
    }
});
...
businessApiApp.get('/appointments', async (req, res) => {
    ...
    try {
        ...
        await db
            ...
            .doc(businessId)
            .update({ usageThisMonth: admin.firestore.FieldValue.increment(1) });
        await logUsage({ businessId, endpoint: 'appointments', ip: req.ip, success: true, latencyMs: Date.now() - req._startTime });
        res.status(200).json({
            appointments,
        ...
        await logUsage({ businessId, endpoint: 'appointments', ip: req.ip, success: false, latencyMs: Date.now() - req._startTime });
        res.status(500).json({ error: 'Internal Server Error' });
    }
});
...
businessApiApp.get('/usage', async (req, res) => {
    ...
    try {
        ...
        await db
            ...
            .doc(businessId)
            .update({ usageThisMonth: admin.firestore.FieldValue.increment(1) });
        await logUsage({ businessId, endpoint: 'usage', ip: req.ip, success: true, latencyMs: Date.now() - req._startTime });
        res.status(200).json({
            month: targetMonth,
            year: targetYear,
            totalCalls,
        ...
        await logUsage({ businessId, endpoint: 'usage', ip: req.ip, success: false, latencyMs: Date.now() - req._startTime });
        res.status(500).json({ error: 'Internal Server Error' });
    }
});
...
businessApiApp.get('/invoices', async (req, res) => {
    ...
    try {
        ...
        await db
            ...
            .doc(businessId)
            .update({ usageThisMonth: admin.firestore.FieldValue.increment(1) });
        await logUsage({ businessId, endpoint: 'invoices', ip: req.ip, success: true, latencyMs: Date.now() - req._startTime });
        res.status(200).json({
            invoices,
        ...
        await logUsage({ businessId, endpoint: 'invoices', ip: req.ip, success: false, latencyMs: Date.now() - req._startTime });
        res.status(500).json({ error: 'Internal Server Error' });
    }
});
...
businessApiApp.get('/account', async (req, res) => {
    ...
    try {
        ...
        await db
            ...
            .doc(businessId)
            .update({ usageThisMonth: admin.firestore.FieldValue.increment(1) });
        await logUsage({ businessId, endpoint: 'account', ip: req.ip, success: true, latencyMs: Date.now() - req._startTime });
        res.status(200).json({
            id: businessId,
        ...
        await logUsage({ businessId, endpoint: 'account', ip: req.ip, success: false, latencyMs: Date.now() - req._startTime });
        res.status(500).json({ error: 'Internal Server Error' });
    }
});
...
exports.resetMonthlyQuotas = (0, scheduler_1.onSchedule)('0 0 1 * *', async () => {
    ...
    for (const doc of activeBusinesses.docs) {
        ...
        if (businessData.usageThisMonth > 0) {
            ...
            const { generateMonthlyInvoice } = await Promise.resolve().then(() => __importStar(require('./billingEngine')));
            try {
