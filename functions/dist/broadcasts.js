 ...
 */
async function getTargetUsers(targetingFilters) {
    try {
 ...
 */
async function sendMessagesInBatches(messageData, targetUsers, messageId) {
    const result = {
        success: true,
        deliveredCount: 0,
        failedCount: 0,
 ...
 */
async function sendBatchWithRetry(users, basePayload, messageId, attempt = 1) {
    const result = {
        success: true,
        deliveredCount: 0,
        failedCount: 0,
    ...
    try {
        ...
        if (response.failureCount > 0) {
            ...
            if (attempt < MAX_RETRY_ATTEMPTS) {
                const retryableUsers = failedUsers.filter(user => isRetryableError(user.error));
                if (retryableUsers.length > 0) {
                    console.log(`Retrying ${retryableUsers.length} failed messages (attempt ${attempt + 1})`);
                    await delay(RETRY_DELAY_MS * attempt);
                    const retryResult = await sendBatchWithRetry(retryableUsers.map(u => ({ id: u.id, fcmToken: u.fcmToken })), basePayload, messageId, attempt + 1);
        ...
        // Track all as failed
        await trackFailedDeliveries(users.map(u => ({ ...u, error: 'batch-send-error' })), messageId);
 ...
 */
function createFCMPayload(messageData, messageId) {
    const payload = {
        notification: {
 ...
 */
async function trackDeliveryEvents(users, messageId, event) {
 ...
 */
async function trackFailedDeliveries(failedUsers, messageId) {
    const batch = firestore.batch();
...
function isRetryableError(error) {
    ...
    return retryableErrors.some(retryableError => error.toLowerCase().includes(retryableError));
}
/**
 * Utility functions
 */
