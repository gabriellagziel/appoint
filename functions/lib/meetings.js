...
function validateMeetingBusinessRules(meeting) {
...
function canAccessEventFeatures(meeting, userId) {
    const meetingType = getMeetingType(meeting.participants);
...
exports.onMeetingWrite = functions.firestore
    ...
    const previousMeeting = change.before.exists ? change.before.data() : null;
...
exports.validateMeetingCreation = functions.https.onCall(async (data, context) => {
    ...
    const totalParticipants = participants.length + 1; // +1 for organizer
    if (totalParticipants < 2) {
        throw new functions.https.HttpsError('invalid-argument', 'Meeting must have at least one participant besides the organizer');
    }
...
exports.checkEventFeatureAccess = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const userId = context.auth.uid;
...
exports.getMeetingAnalytics = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const userId = context.auth.uid;
...
exports.createEventForm = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const userId = context.auth.uid;
    ...
    const meetingType = getMeetingType(meeting.participants);
    if (meetingType !== 'event') {
        throw new functions.https.HttpsError('failed-precondition', 'Custom forms are only available for events (4+ participants)');
    }
    if (!canAccessEventFeatures(meeting, userId)) {
        throw new functions.https.HttpsError('permission-denied', 'Only event organizers and admins can create forms');
    }
    // Create form
...
exports.cleanupExpiredMeetings = functions.pubsub
...
// Helper functions
async function cleanupEventFeatures(meetingId, meeting) {
    const batch = db.batch();
...
async function initializeEventFeatures(meetingId) {
    const eventSettings = {
...
async function updateMeetingTypeAnalytics(userId, newType, oldType) {
...
async function handleParticipantChanges(currentMeeting, previousMeeting) {
    ...
    const addedParticipants = currentMeeting.participants.filter(p => !previousParticipantIds.has(p.userId));
    // Find removed participants
    const removedParticipantIds = Array.from(previousParticipantIds).filter(id => !currentParticipantIds.has(id));
    // Send notifications to added participants
    for (const participant of addedParticipants) {
        await sendNotificationToUser(participant.userId, 'Meeting Invitation', `You have been added to ${currentMeeting.title}`);
    }
