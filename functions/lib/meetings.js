...
export const onMeetingWrite = onDocumentWritten('meetings/{meetingId}', async (event) => {
    ...
    const meeting = event.data?.after?.data();
    const previousMeeting = event.data?.before?.exists ? event.data?.before?.data() : null;
...
export const validateMeetingCreation = onCall(async (request) => {
    ...
    const totalParticipants = participants.length + 1; // +1 for organizer
    if (totalParticipants < 2) {
        throw new Error('Meeting must have at least one participant besides the organizer');
    }
...
export const createEventForm = onCall(async (request) => {
    ...
    const meetingType = getMeetingType(meeting.participants);
    if (meetingType !== 'event') {
        throw new Error('Custom forms are only available for events (4+ participants)');
    }
    if (!canAccessEventFeatures(meeting, userId)) {
        throw new Error('Only event organizers and admins can create forms');
    }
    // Create form
