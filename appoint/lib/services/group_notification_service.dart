...
class GroupNotificationService {
  ...
  Future<void> notifyGroupMembers(String groupId, GroupNotificationType type, {
  ...
  Future<void> notifyGroupAdmins(String groupId, GroupNotificationType type, {
    required String title,
    required String message,
    Map<String, dynamic>? data,
    String? performedBy,
  ...
  Future<void> notifyUser(String userId, GroupNotificationType type, {
    required String title,
    required String message,
    Map<String, dynamic>? data,
    String? performedBy,
  ...
  /// קבלת התראות לפי סוג
  Map<String, int> _groupNotificationsByType(List<Map<String, dynamic>> notifications) {
  ...
  Future<void> notifyMemberJoined(String groupId, String memberId, String performedBy) async {
    final group = await _groupSharingService.getGroupById(groupId);
    if (group == null) return;

    await notifyGroupAdmins(groupId, GroupNotificationType.memberJoined,
      title: 'חבר חדש הצטרף',
  ...
  Future<void> notifyMemberLeft(String groupId, String memberId, String performedBy) async {
    final group = await _groupSharingService.getGroupById(groupId);
    if (group == null) return;

    await notifyGroupAdmins(groupId, GroupNotificationType.memberLeft,
      title: 'חבר עזב',
  ...
  Future<void> notifyAdminAdded(String groupId, String adminId, String performedBy) async {
    final group = await _groupSharingService.getGroupById(groupId);
    if (group == null) return;

    await notifyUser(adminId, GroupNotificationType.adminAdded,
      title: 'הרשאות מנהל',
  ...
  Future<void> notifyInviteCancelled(String groupId, String inviteCode, String performedBy) async {
    final group = await _groupSharingService.getGroupById(groupId);
    if (group == null) return;

    await notifyGroupAdmins(groupId, GroupNotificationType.inviteCancelled,
      title: 'הזמנה בוטלה',
