...
class FrameTimeTest {
  static const Duration _frameBudget = Duration(milliseconds: 16); // 60 FPS target
  static const Duration _jankThreshold = Duration(milliseconds: 16); // Frames > 16ms are jank
  ...
  static FrameTimeResult _analyzeFrameTimes(
    ...
    final frameTimeMs = frameTimes.map((d) => d.inMicroseconds / 1000.0).toList();
    ...
    final variance = frameTimeMs.map((t) => (t - avgTime) * (t - avgTime)).reduce((a, b) => a + b) / frameTimeMs.length;
  ...
  static List<PerformanceIssue> detectPerformanceIssues(FrameTimeResult result) {
    ...
    if (result.jankPercentage > 5.0) {
      issues.add(PerformanceIssue(
        ...
        description: 'High jank detected: ${result.jankPercentage.toStringAsFixed(1)}% of frames exceed 16ms',
        recommendation: 'Optimize rendering pipeline and reduce frame time spikes',
      ));
    }
    ...
    if (result.averageFrameTime.inMilliseconds > 16) {
      issues.add(PerformanceIssue(
        ...
        description: 'Average frame time (${result.averageFrameTime.inMilliseconds}ms) exceeds 16ms target',
        recommendation: 'Optimize widget rebuilds and reduce computational complexity',
      ));
    }
    ...
    if (result.standardDeviation > 8.0) {
      issues.add(PerformanceIssue(
        ...
        severity: PerformanceIssueSeverity.medium,
        description: 'High frame time variance (${result.standardDeviation.toStringAsFixed(2)}ms)',
...
class FrameTimeBenchmark {
  ...
  double get overallJankPercentage {
    ...
    final totalJank = results.values.map((r) => r.jankCount).reduce((a, b) => a + b);
    final totalFrames = results.values.map((r) => r.totalFrames).reduce((a, b) => a + b);
...
void main() {
  group('Frame Time Performance Tests', () {
    testWidgets('should maintain smooth 60 FPS during navigation', (WidgetTester tester) async {
      ...
      expect(result.isSmooth, isTrue,
          reason: 'Navigation is not smooth: ${result.jankPercentage.toStringAsFixed(1)}% jank');
      
      expect(result.averageFPS, greaterThan(55.0),
          reason: 'Average FPS (${result.averageFPS.toStringAsFixed(1)}) is below 55 FPS');
    });
    
    testWidgets('should maintain smooth 60 FPS during scrolling', (WidgetTester tester) async {
      final result = await FrameTimeTest.measureScrollingFrameTimes();
      
      print('Scrolling Frame Time Result: $result');
      
      expect(result.isSmooth, isTrue,
          reason: 'Scrolling is not smooth: ${result.jankPercentage.toStringAsFixed(1)}% jank');
      
      expect(result.averageFPS, greaterThan(55.0),
          reason: 'Average FPS (${result.averageFPS.toStringAsFixed(1)}) is below 55 FPS');
    });
    
    testWidgets('should maintain smooth 60 FPS during animations', (WidgetTester tester) async {
      final result = await FrameTimeTest.measureAnimationFrameTimes();
      
      print('Animation Frame Time Result: $result');
      
      expect(result.isSmooth, isTrue,
          reason: 'Animations are not smooth: ${result.jankPercentage.toStringAsFixed(1)}% jank');
      
      expect(result.averageFPS, greaterThan(55.0),
          reason: 'Average FPS (${result.averageFPS.toStringAsFixed(1)}) is below 55 FPS');
    });
    
    testWidgets('should handle data loading without excessive jank', (WidgetTester tester) async {
      ...
      expect(result.jankPercentage, lessThan(10.0),
          reason: 'Data loading has excessive jank: ${result.jankPercentage.toStringAsFixed(1)}%');
      
      expect(result.averageFPS, greaterThan(45.0),
          reason: 'Average FPS (${result.averageFPS.toStringAsFixed(1)}) is too low during data loading');
    });
    
    testWidgets('should pass comprehensive frame time benchmark', (WidgetTester tester) async {
      ...
      expect(benchmark.overallJankPercentage, lessThan(5.0),
          reason: 'Overall jank percentage (${benchmark.overallJankPercentage.toStringAsFixed(1)}%) is too high');
    });
    
    testWidgets('should detect performance issues correctly', (WidgetTester tester) async {
      // Create a result with known issues
      final problematicResult = FrameTimeResult(
        averageFrameTime: Duration(milliseconds: 20),
        minFrameTime: Duration(milliseconds: 10),
        maxFrameTime: Duration(milliseconds: 50),
        jankPercentage: 15.0,
        jankCount: 15,
        totalFrames: 100,
        isSmooth: false,
      ...
      expect(issues.any((i) => i.type == PerformanceIssueType.slowFrames), isTrue,
