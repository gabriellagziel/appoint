...
class StartupTimeTest {
  ...
  static Future<StartupTimeResult> measureStartupTime() async {
    ...
    try {
      // Start the app
      final app = await IntegrationTestWidgetsFlutterBinding.ensureInitialized();
  ...
  static StartupTimeAnalysis analyzeResults(List<StartupTimeResult> results) {
    ...
    final startupTimes = results.map((r) => r.totalStartupTime.inMilliseconds).toList();
    ...
    final variance = startupTimes.map((t) => (t - avgTime) * (t - avgTime)).reduce((a, b) => a + b) / startupTimes.length;
...
void main() {
  group('Startup Time Performance Tests', () {
    testWidgets('should start within 2 seconds', (WidgetTester tester) async {
      final results = await StartupTimeTest.runStartupTimeBenchmark(iterations: 3);
      ...
      expect(analysis.isWithinBudget, isTrue, 
          reason: 'Average startup time (${analysis.averageStartupTime.inMilliseconds}ms) '
                  'exceeds budget (2000ms)');
      
      expect(analysis.passRate, greaterThan(0.8), 
          reason: 'Pass rate (${(analysis.passRate * 100).toStringAsFixed(1)}%) '
                  'is below 80%');
    });
    
    testWidgets('should have consistent startup times', (WidgetTester tester) async {
      final results = await StartupTimeTest.runStartupTimeBenchmark(iterations: 5);
      final analysis = StartupTimeTest.analyzeResults(results);
      
      // Check for consistency (low standard deviation)
      expect(analysis.standardDeviation, lessThan(500.0), 
          reason: 'Standard deviation (${analysis.standardDeviation.toStringAsFixed(2)}ms) '
                  'is too high, indicating inconsistent startup times');
      
      // Check that min and max are within reasonable range
      final range = analysis.maxStartupTime.inMilliseconds - analysis.minStartupTime.inMilliseconds;
      expect(range, lessThan(1000.0), 
          reason: 'Startup time range (${range}ms) is too large');
    });
    
    testWidgets('should complete first frame quickly', (WidgetTester tester) async {
      final result = await StartupTimeTest.measureStartupTime();
      
      expect(result.firstFrameTime.inMilliseconds, lessThan(500), 
          reason: 'First frame time (${result.firstFrameTime.inMilliseconds}ms) '
                  'exceeds 500ms threshold');
    });
    
    testWidgets('should reach interactive state quickly', (WidgetTester tester) async {
      final result = await StartupTimeTest.measureStartupTime();
      
      expect(result.interactiveTime.inMilliseconds, lessThan(1000), 
          reason: 'Interactive time (${result.interactiveTime.inMilliseconds}ms) '
