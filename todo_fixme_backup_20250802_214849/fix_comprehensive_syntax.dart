  ...
  for (final file in dartFiles) {
    try {
      ...
      content = content.replaceAll(
          RegExp(r'class ([a-zA-Z_][a-zA-Z0-9_]*);'), 'class \$1 {');

      // Fix 2: Fix malformed class declarations
      content = content.replaceAll(
          RegExp(r'class ([a-zA-Z_][a-zA-Z0-9_]*) {([^}]*)}'),
          'class \$1 {\n  \$2\n}');
      ...
      content = content.replaceAll(
          ...
          'final \$1 = \$2;');
      content = content.replaceAll(
          RegExp(r'const ([a-zA-Z_][a-zA-Z0-9_]*) = ([^;]+)(?!;)'),
          'const \$1 = \$2;');
      content = content.replaceAll(
          RegExp(r'static ([a-zA-Z_][a-zA-Z0-9_]*) = ([^;]+)(?!;)'),
          'static \$1 = \$2;');
      ...
      content = content.replaceAll(
          RegExp(r'required this\.([a-zA-Z_][a-zA-Z0-9_]*)'), 'required \$1');
      content =
          content.replaceAll(RegExp(r'this\.([a-zA-Z_][a-zA-Z0-9_]*)'), '\$1');

      // Fix 7: Fix malformed imports
      content =
          content.replaceAll(RegExp(r'import ([^;]+)(?!;)'), 'import \$1;');
      content =
          content.replaceAll(RegExp(r'export ([^;]+)(?!;)'), 'export \$1;');
      content = content.replaceAll(RegExp(r'part ([^;]+)(?!;)'), 'part \$1;');

      // Fix 8: Fix function expressions
      content = content.replaceAll(
          RegExp(r'([a-zA-Z_][a-zA-Z0-9_]*) => ([^;]+)'), '(\$1) => \$2');

      // Fix 9: Fix missing semicolons after method calls
      content = content.replaceAll(
          RegExp(r'([a-zA-Z_][a-zA-Z0-9_]*\([^)]*\))(?!;)'), '\$1;');
      ...
      if (content.contains('@JsonSerializable()') &&
        ...
        final className = RegExp(r'class ([a-zA-Z_][a-zA-Z0-9_]*)')
            ...
            ?.group(1);
        if (className != null) {
          content = content.replaceFirst('@JsonSerializable()',
              '@JsonSerializable()\npart \'${className.toLowerCase()}.g.dart\';');
        }
      }
      ...
      content = content.replaceAll(
          RegExp(r'([a-zA-Z_][a-zA-Z0-9_]*)\s*\(([^)]*)\)\s*{\s*([^}]*)\s*}'),
          '\$1(\$2) {\n    \$3\n  }');
      ...
      content = content.replaceAll(
          RegExp(r'([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*([^;]+)(?!;)'), '\$1 = \$2;');
