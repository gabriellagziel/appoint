...
describe('Admin Service Health Checks', () => {
  let app;
  ...
  describe('GET /health.html', () => {
    it('should return health check page with 200 status', async () => {
      ...
      expect(response.text).toContain('Admin service is healthy');
  ...
  describe('GET /', () => {
    it('should serve the main admin application page', async () => {
      const response = await request(app)
        .get('/')
        .expect(200);

      expect(response.text).toContain('App-Oint Admin Panel');
      expect(response.text).toContain('Admin Service Online');
    });
  ...
  describe('Admin Panel Specific Tests', () => {
    it('should serve admin-specific content', async () => {
      const response = await request(app)
        .get('/')
        .expect(200);

      expect(response.text).toContain('Administrative Dashboard');
      expect(response.text).toContain('Admin panel is running successfully');
    });

    it('should contain admin panel identifier', async () => {
      const response = await request(app)
        .get('/')
        .expect(200);

      expect(response.text).toContain('⚙️ App-Oint Admin Panel');
    });
  });
  ...
  describe('SPA Routing Support', () => {
    ...
    it('should handle admin deep routes', async () => {
      const response = await request(app)
    ...
    it('should handle dashboard routes', async () => {
      const response = await request(app)
  ...
  describe('HTTP Methods', () => {
    ...
    it('should handle OPTIONS for admin routes', async () => {
      await request(app)
        .options('/')
        .expect(200);
    });
  ...
  describe('Content Validation', () => {
    it('should contain expected admin keywords in main page', async () => {
      const response = await request(app)
        .get('/')
        .expect(200);
      ...
      const expectedKeywords = [
        ...
        'Admin panel is running successfully'
      ];
    ...
    it('should have admin-specific styling', async () => {
      const response = await request(app)
        .get('/')
        .expect(200);

      // Check for admin gradient colors
      expect(response.text).toContain('#fc466b');
      expect(response.text).toContain('#3f5efb');
    });
  });
  ...
  describe('Error Scenarios', () => {
    ...
    it('should handle requests with special characters', async () => {
      await request(app)
        .get('/admin?param=value&other=<script>')
        .expect(200);
    });

    it('should handle encoded admin routes', async () => {
      await request(app)
        .get('/admin%2Fusers%2Flist')
        .expect(200);
    });
  });
  ...
  describe('Health Check Integration', () => {
    ...
    it('should support admin service monitoring', async () => {
      const response = await request(app)
        .get('/health.html')
        .expect(200);
  ...
  describe('Admin Panel Security Headers', () => {
    it('should handle security-related headers gracefully', async () => {
      const response = await request(app)
        .get('/')
    ...
    it('should serve admin content regardless of user agent', async () => {
      const response = await request(app)
        .get('/')
