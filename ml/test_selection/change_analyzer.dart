...
class ChangeAnalyzer {
  ...
  static Future<ChangeImpact> analyzeChangeImpact({
    ...
    try {
      ...
      final coverageImpact = await _calculateCoverageImpact(filePath, impactedTests);
      
      // Determine priority
      final priority = _determinePriority(riskScore, coverageImpact, impactedTests.length);
  ...
  static ChangeType _determineChangeType(String filePath, List<String> changedLines) {
  ...
  static List<String> _getCoverageImpactedTests(String filePath) {
    ...
    if (filePath.contains('/services/')) {
      tests.add('test/services/${path.basename(filePath).replaceAll('.dart', '_test.dart')}');
    }
    
    if (filePath.contains('/models/')) {
      tests.add('test/models/${path.basename(filePath).replaceAll('.dart', '_test.dart')}');
    }
    
    if (filePath.contains('/providers/')) {
      tests.add('test/providers/${path.basename(filePath).replaceAll('.dart', '_test.dart')}');
    }
