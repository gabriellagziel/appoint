...
class TestPrioritizer {
  static const String _executionHistoryFile = 'ml/data/test_execution_history.json';
  ...
  Future<List<TestPriorityResult>> prioritizeTests({
    ...
    try {
      ...
      for (final testName in allTests) {
        ...
        final priority = _determineFinalPriority(combinedScore, prediction.confidence);
  ...
  Future<void> _loadExecutionHistory() async {
    try {
      ...
      for (final entry in data.entries) {
        ...
        _executionHistory[testName] = TestExecutionHistory.fromJson(historyData);
      }
    } catch (e) {
  ...
  Future<TestFeatureVector> _extractTestFeatures(
    ...
    final avgExecutionTime = history?.averageExecutionTime.inMilliseconds ?? 1000;
    final lastExecutionTime = history?.lastExecutionTime ?? DateTime.now().subtract(Duration(days: 30));
    ...
    return TestFeatureVector(
      ...
      daysSinceLastExecution: DateTime.now().difference(lastExecutionTime).inDays,
  ...
  Future<TestPrediction> _predictTestPriority(TestFeatureVector features) async {
    try {
  ...
  TestPrediction _manualPrediction(TestFeatureVector features) {
    ...
    return TestPrediction(
      ...
      relevanceScore: features.isDirectlyImpacted * 0.8 + features.riskScore * 0.2,
    );
  }
  ...
  double _encodeChangeType(ChangeType changeType) {
    switch (changeType) {
      case ChangeType.structural: return 0.9;
      case ChangeType.functional: return 0.7;
      case ChangeType.dependency: return 0.5;
      case ChangeType.logic: return 0.6;
      case ChangeType.test: return 0.2;
      case ChangeType.configuration: return 0.3;
      case ChangeType.data: return 0.4;
      case ChangeType.documentation: return 0.1;
      case ChangeType.other: return 0.3;
    }
  }
  ...
  double _encodePriority(TestPriority priority) {
    switch (priority) {
      case TestPriority.critical: return 0.9;
      case TestPriority.high: return 0.7;
      case TestPriority.medium: return 0.5;
      case TestPriority.low: return 0.3;
    }
  }
...
class TestExecutionHistory {
  ...
  Duration get averageExecutionTime {
    if (executionTimes.isEmpty) return Duration(milliseconds: 1000);
    final totalMs = executionTimes.fold<int>(0, (sum, time) => sum + time.inMilliseconds);
